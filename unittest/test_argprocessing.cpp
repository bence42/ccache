...
TEST_CASE("ti_compiler_flags")
{
  ...
  Context ctx;
  ctx.orig_args = Args::from_string("cl6x --compile_only --cpp_file=foo.c --output_file=foo.c.o");
  ctx.config.set_compiler_type(CompilerType::ti);
...
TEST_CASE("ti_dash_dash_preproc_dependency_should_work_like_dash_MF")
{
  ...
  Context ctx;
  ctx.orig_args = Args::from_string("cl6x --preproc_with_compile --cpp_file=foo.c --output_file=foo.o --preproc_dependency=foo.o.d");
  ctx.config.set_compiler_type(CompilerType::ti);
  ...
  CHECK(!result.error);
  CHECK(result.preprocessor_args.to_string() == "cl6x");
  CHECK(result.extra_args_to_hash.to_string() == "--preproc_with_compile --preproc_dependency=foo.o.d");
  CHECK(result.compiler_args.to_string() == "cl6x --preproc_with_compile -c --preproc_dependency=foo.o.d");
  CHECK(ctx.args_info.input_file == "foo.c");
